...
class ReactiveContradictionAnalyzer:
    ...
    def compute_propagation_priority(self, delta: Delta, affected_keys: Set[str]) -> float:
        """
...
class ReactiveContractionAnalyzer:
    ...
    def compute_propagation_efficiency(self, delta: Delta, affected_keys: Set[str]) -> float:
        """
...
class DeltaKVStore:
    ...
    def _propagate_change(self, delta: Delta) -> None:
        ...
        if len(affected_keys) > 2:  # Lower threshold for complex cases
            ...
            for computed_key in self._computed.keys():
                if computed_key in self._dep_graph._reverse_graph:
                    dependencies[computed_key] = self._dep_graph._reverse_graph[computed_key]
            self._contradiction_analyzer.analyze_dependency_conflicts(dependencies)
            
            propagation_priority = self._contradiction_analyzer.compute_propagation_priority(
                delta, affected_keys
            )
        ...
        if use_nof_optimization:
            ...
            propagation_order = self._get_optimized_propagation_order(affected_keys, propagation_priority)
            # Track NOF optimization usage
            self._nof_optimizations_used = getattr(self, '_nof_optimizations_used', 0) + 1
        else:
    ...
    def _get_optimized_propagation_order(self, affected_keys: Set[str], priority: float) -> List[str]:
        """
        ...
        if priority > 0.5:
            ...
            def contradiction_score(key: str) -> float:
                if key in self._contradiction_analyzer._block_gadgets:
                    ...
                    return block.k_budget  # Higher k_budget = more contradictions resolved
                return 0.0
    ...
    def get_nof_analytics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            k_budget = self._contradiction_analyzer.analyze_dependency_conflicts(dependencies)
            ...
            return {
                ...
                "dependency_conflicts": len(self._contradiction_analyzer._block_gadgets),
                "propagation_history_size": len(self._contraction_analyzer._propagation_history),
