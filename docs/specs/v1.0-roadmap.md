...
> **Archived design draft:** This file records an older v1.0 exploration and is
> not the current public API. The implemented FynX algebra uses `>>`
> / `.then()` for transforms, `+` / `.alongside()` for products, `&`
> / `.requiring()` for filters, `|` / `.either()` for logical OR, and `~`
> / `.negate()` for negation. Current transform functions are pure and must use
> their explicit arguments; extra observable inputs should be combined first.
> See `README.md` and `docs/generation/markdown/` for maintained documentation.
