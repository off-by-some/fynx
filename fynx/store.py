...
#     # Create a direct observable that bypasses Observable overhead
...
class StoreMeta(type):
    """Metaclass for Store classes that handles observable descriptors."""

    def __new__(cls, name, bases, namespace, **kwargs):
        # Find observable attributes
        observable_attrs = []
        observables = {}

        for attr_name, attr_value in namespace.items():
            if hasattr(attr_value, "_is_observable_descriptor"):
                observable_attrs.append(attr_name)
                observables[attr_name] = attr_value

        # Store metadata on the class
        namespace["_observable_attrs"] = observable_attrs
        namespace["_observables"] = observables
    ...
    def __setattr__(cls, name: str, value: Any) -> None:
        """Intercept class attribute assignment for observables."""
        if hasattr(cls, "_observables") and name in getattr(cls, "_observables", {}):
            # It's an observable attribute, delegate to its set method
            cls._observables[name].set(value)
        else:
            super().__setattr__(name, value)

    def __getattribute__(cls, name: str) -> Any:
        """Intercept class attribute access for observables."""
        # Get the attribute normally
        attr = super().__getattribute__(name)

        # If it's an observable descriptor, return its value
        if hasattr(attr, "_is_observable_descriptor"):
            return attr.value
...
class Store(metaclass=StoreMeta):
    ...
    Store classes can define observable attributes using the observable() descriptor,
    and automatically provide convenient access and subscription methods.
    ...
    def subscribe(cls, callback):
        ...
        def on_change(delta):
            # Call the callback with the store class for context
            callback(cls)

        # Subscribe to all observables
        for obs in cls._observables.values():
            obs.subscribe(on_change)


class ObservableDescriptor:
    """Descriptor that creates observable attributes on Store classes."""

    def __init__(self, initial_value=None):
        self._initial_value = initial_value
        self._observable = None
        self._is_observable_descriptor = True

    def __get__(self, instance, owner):
        """Get the observable value."""
        if instance is None:
            # Class access - return the descriptor itself for StoreMeta to handle
            return self

        # This shouldn't happen for class-level descriptors
        return self

    def __set__(self, instance, value):
        """Set shouldn't be called on descriptors."""
        pass
    ...
    def value(self):
        ...
        if self._observable is None:
            ...
            self._observable = global_observable(self._initial_value)
...
def observable(initial_value=None):
    """
    Create an observable descriptor for use in Store classes.

    This function returns a descriptor that can be used as a class attribute
    in Store subclasses to create reactive properties.

    Args:
        initial_value: The initial value for the observable

    Returns:
        An ObservableDescriptor instance
    ...
    """
    return ObservableDescriptor(initial_value)
